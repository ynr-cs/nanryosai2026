...
> [!IMPORTANT]
> **更新前の絶対義務**:
> AIは `CHANGELOG.md` を更新する前に、**必ずファイルの冒頭80行を読み込み**、以下の3点を例外なく確認しなければなりません。
>
...
CHANGELOGへの詳細集約 (Changelog Centralization): 全ての技術的知見を CHANGELOG.md に集約し、SSOT（唯一の真実）を維持してください。

### 5. Git コミットとプッシュ (Git Commit & Push)

タスク完了後、ユーザーから**明示的に「コミットして」「プッシュして」等と指示された場合にのみ**、以下の手順でGitコマンドを実行してください。ユーザーからの指示がない限り、AIから自発的にGit操作（コミットやプッシュ）を行ってはなりません。

**Git運用ルール**:
- コミットを作成する場合、コミットメッセージは必ず `CHANGELOG.md` で採番した **現在のパッチバージョン番号のみ**（例: `0.5.24`）とすること。
- **【重要】** プレースホルダーの括弧（`[` や `]`）も含めず、純粋な数字とドットのみの文字列（例: `"0.5.24"`）としてください。それ以外の説明文や補足事項は絶対に含めないこと。
- `run_command` を使って `git add .`、`git commit -m "バージョン番号"`、`git push` を実行する。
