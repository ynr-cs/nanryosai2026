...
1. **試行錯誤のログ = パッチ (Trial and Error = Patch)**: 開発中のあらゆる変更（バグ修正、機能追加、調整）は、原則としてパッチ番号（例: v0.2.1 → v0.2.2）としてインクリメントし、ログとして蓄積してください。統合されて実用できる状態になるまでは、どれだけ大きな変更でもパッチとして扱います。
2. **マイナーバージョンの厳格管理 (Strict Minor Versioning)**: マイナーバージョン (0.x.0) を上げるのは、ユーザーが「完全に使える状態になった」「これで一区切りついた」と判断した時のみです。
3. **メジャーバージョンの基準 (Major Versioning)**: メジャーバージョン (x.0.0) を上げるのは、ユーザーがすべてのファイルを精査し、「南稜祭本番で稼働できる」と最終判断を下した時のみです。
4. **提案と承認プロセス (Propose & Approve)**: AIは進捗に応じてマイナーアップデートを提案できますが、ユーザーの明示的な「承認」がない限り、勝手にバージョンを上げることは禁止します。
5. **CHANGELOGへの詳細集約 (Changelog Centralization)**: 作業で得た技術的知見、バグの原因、解決策は、分散させず全て `CHANGELOG.md` に詳細に書き込むこと。新しい会話で過去のパッチを読み込むだけで状況が把握できる状態を目指す。
6. **全言語日本語**: ユーザーへのメッセージ、作成するドキュメント（コード内の例外や識別子を除く）は全て日本語で行ってください。
7. **アーティファクト管理**: `agentic_mode` の標準アーティファクト (`task.md`, `implementation_plan.md`) は**一時的な作業用**として使用してください。
8. **知識の永続化**: 重要な仕様は `antigravity` フォルダ内の `_CONTEXT.md` にも反映するが、デバッグ知見は `CHANGELOG.md` を主軸とする。
9. **ダークモード対応**: 全てのUI変更において、ライト/ダーク両モードでの視認性を確認してください。
10. **レスポンシブ (Mobile First)**: PCだけでなく、スマートフォン・タブレットでの表示崩れがないか確認してください。
11. **既存資産の再利用**: 新規CSS記述の前に、既存のデザインシステム（変数・クラス）の利用を優先してください。
