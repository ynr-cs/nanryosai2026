...
4.  **変更履歴の更新**: タスクの最後に、必ず `CHANGELOG.md` を更新してください。
5.  **ダークモード対応**: 全てのUI変更において、ライト/ダーク両モードでの視認性を確認してください。
6.  **レスポンシブ (Mobile First)**: PCだけでなく、スマートフォン・タブレットでの表示崩れがないか確認してください。
7.  **既存資産の再利用**: 新規CSS記述の前に、既存のデザインシステム（変数・クラス）の利用を優先してください。
...
タスクの最後に、ルートディレクトリの `CHANGELOG.md` に今回の変更を追記してください。

**フォーマット:**

```markdown
## [バージョン番号] バージョン名 - YYYY-MM-DD

### メタ情報

- **時刻**: HH:MM
- **AIモデル**: [使用中のモデル名] (例: Gemini 3.0 Pro)
- **筆者**: [AI or User]
