...
- **差分同期（Upsert方式）**: 指定店舗 (`storeId`) に紐づく `items` ドキュメントは、**完全削除を行わず差分更新**されます。
  - 既存のドキュメントIDおよび商品名に一致するアイテムは、IDを不変に維持したままフィールド情報（価格・トッピング・説明文）をマージ更新（`merge: true`）します。
  - これにより、文化祭営業時間中にメニュー情報を同期しても**来場者のカート内商品が無効化されず、注文受付が停止しません**。
  - 店舗側がポータルで設定した売切ステータス（`isAvailable: false`）やアップロード済み写真URL（`imageUrl`）も自動保護されます。
- **価格データ**: 文字列（`"300円"`）から数値（`300`）への変換が行われます。
- **商品画像**: `data.js` 上で `imageUrl` が空文字（`""`）であっても、Firestore に既存写真URLがある場合は自動維持されます。明示的な画像削除（`null`）時のみリンクが解除されます。
...
- `admin_sync.html` のメニュー同期は差分同期（Upsert）に対応したため、`data.js` の `imageUrl` が空文字であっても**Firestore上のアップロード済み写真URLが誤って消えることはありません**。
- **推奨運用フロー**:
  1. `data.js` でメニューの基本情報（名前・価格）を定義＆同期。
  2. 各店舗がポータル (`pos/portal.html`) にログインし、写真をアップロードする、あるいは管理者が `admin_sync.html` からアップロードする。
  3. `admin_sync.html` で「現在のモードから読込」または「本番から読込」を実行すると、**非破壊マージにより全26企画の一般企画を保護したまま**、最新の写真URLやドキュメントIDを安全にローカルへ取り込むことができます。
