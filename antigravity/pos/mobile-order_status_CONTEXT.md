...
- **機能**:
  ...
  - ステータス可視化（設計憲法§1.1準拠: 調理中 / 準備中 / お呼出 / 完了 / キャンセル / 期限切れ破棄）。
  - 呼出番号 (Receipt Number) の表示。
  - 呼び出し時の5分間受渡期限カウントダウンタイマー。
- **モバイル端末向け多層防御同期リカバリ機構 (2026-09-05 実装)**:
  - **背景**: Android Chrome 等のモバイル端末において、画面消灯（スリープ）やバックグラウンド移行に伴い WebSocket が一時切断・サスペンドされた際、画面復帰後にリスナーがゾンビ化し「調理中」のまま自動更新されない事象が発生していた。
  - **1. ライフサイクル復帰トリガー**:
    - `document.visibilityState === "visible"`（スリープ解除、タブ復帰）および `window.focus` 時に、明示的な `getDoc` による即時データ同期（`syncOrderStatus`）を実行。
    - `window.online` 時に即時同期およびリスナーの再起動（`startListener`）を実行。
  - **2. アクティブ注文セーフティ・ポーリング**:
    - 調理中・準備中・呼出中の進行中注文に限り、フォアグラウンド表示時に 15 秒間隔で最新データを定期取得（`startSafetyPolling`）。
    - 完了・キャンセル・破棄などの終端ステータス遷移時に自動停止し不要なリソース消費を防止。
  - **3. 手動「更新」ボタン & 同期インジケーター**:
    - ステータスカード上部にリアルタイム接続状態（パルスドット）と最終更新時刻（例: `最終更新 12:34:56`）、および手動「更新」ボタンを配置。
    - ユーザーによる任意の再同期（回転アニメーション付き）を可能にし、回線不調時でも確実に復旧できる手段を提供。
  - **4. リスナー重複管理**:
    - `unsubscribeSnapshot` を保持し、認証状態の変化や再接続時に既存リスナーを確実に解除してから再購読を行い、メモリリークと多重発火を防止。
...
## 6. 店舗運用全体フロー概略（3系統注文〜厨房〜受渡）

現場向けチートシートとして `設計図/sandbox/mobile_order_overview.typ`（A4・1枚）を整備しています。

### 業務フロー概要
1. **STEP 1: 3つの注文窓口（インプット）**
   - **校内モバイルオーダー**: 生徒が自端末からどこでも事前注文（行列回避）。
   - **店頭セルフオーダー（キオスク）**: 一般来場者が店頭iPadでタッチ注文。
   - **店頭有人POSレジ**: 操作サポートが必要な来場者向けにスタッフが代理入力。
2. **STEP 2: 厨房・焼きブース（調理）**
   - 全注文がリアルタイムに厨房iPad（キッチンモニター）に自動集約表示。
   - 注文（例: #101 プレーン2、チョコ1）を確認してカステラを焼き上げ、カップに詰めて受渡口の完成品置き場へ置く。
3. **STEP 3: 受渡口・プレゼンター（仕上げ・提供）**
   - 注文に合わせてトッピング（チョコソース、抹茶ソース）を投入。プレーンはそのまま。
   - 「注文番号 #〇〇 のお客様！」と肉声でお呼び出し（呼出モニターにも表示）。
   - AirPay端末でサッと決済し、商品をお渡しして完了。
