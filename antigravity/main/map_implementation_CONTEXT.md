...
v0.2.0 (2026-02-05) より、建物の表現形式を「単純な矩形」から「自由な多角形」へ移行しました。また、v0.3.0より「道路」データが追加されています。
...
### 🛣️ Road Object Schema

v0.3.0 追加。道路は「幅を持った線（リボン）」として表現されます。
建物と異なり、`x`, `z`, `rotation` プロパティを持ちません（パス自体がワールド座標）。
