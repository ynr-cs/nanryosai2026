...
## 10. 通知許可フローの順序最適化（規約同意後・店舗選択前）(v0.5.236)

1. **認知的UXの適正化**:
   - 従来は起動直後にいきなり通知許可画面（`step-notification`）が表示されていたが、ユーザーが「何のアプリで、どういう流れで使うのか」を理解する前に権限を求められるため、拒否（ブロック）されるリスクが高かった。
   - 起動直後はまずウォークスルー（5画面）を表示し、使い方・受け渡し・重要規約を理解・同意させる。
2. **規約同意後に通知設定を案内**:
   - スライド5で規約に同意した直後に、未決定（`Notification.permission === "default"` かつ非iOS）の場合のみ通知設定画面（`step-notification`）を表示。
