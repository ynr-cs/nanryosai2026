...
| **共通UI** | 緊急アラートバナー | **`map.html`, `map3d.html` では表示除外** | マップ画面上部の検索・フロア切替UIおよびマップビューが隠れて操作不能になるのを防止するため。 |
...
4. **コンピュータ科学部（`id=cs`）のリダイレクト**: `detail.html?id=cs`（大文字小文字問わず）を開いた場合、自動的に部活紹介専用ページ `about-us.html` へ即時シームレス遷移。
...
1. **メインナビゲーション（枠線なしクリーンな縦リスト）**:
   - 開催概要 (`about.html`), 企画一覧 (`projects-list.html`), ステージ発表 (`stage-list.html`), 校内マップ (`map.html`), アクセス (`access.html`)
   - 枠線を排除し、余白とホバー演出で洗練されたシンプルな一覧UI。
2. **「その他のサービス」アコーディオン（`.menu-accordion-section`）**:
   - 初期状態で折りたたまれたアコーディオン。タップで開くと、マイページ (`account.html`), 生徒会人気投票（アンケート外部リンク）, モバイルオーダー (`mobile-order.html`), アンケート（外部リンク）がはみ出さないコンパクトな2列ピル型ボタンで展開。
3. **最下部に完全固定されたテーマ設定＆フッター（`.menu-bottom-container`）**:
   - `margin-top: auto` により、アコーディオンの開閉に左右されず常に一番下（Computer科学部の直上）に位置固定。
   - テーマ設定セレクター（ライト / ダーク / 自動）と利用規約・プライバシーリンク行、および「Powered By コンピュータ科学部」を一体化。

### 3.8 プライバシーポリシー（`main/privacy.html`）
1. **お問い合わせ窓口**:
   - 実行委員会表記を削除し、コンピュータ科学部公式お問い合わせGoogleフォームへのリンクボタンを設置。
