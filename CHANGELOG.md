...
## [0.2.135] Venue UI: Design Unification & Centering - 2026-04-20
### メタ情報
...
### 変更 (Changed)
- **`main/index.html`**:
  - **デザインの統一**: リアルタイム情報のセクションヘッダーを「企画・展示ハイライト」と同じスタイル（縦棒付きのタイトル）に変更し、全体の統一感を持たせました。
  - **テキストの中央揃え**: 3列グリッドの狭い表示幅でも見やすくなるよう、団体名、イベントタイトル、時間をすべて中央揃えに調整。
  - **カード構造の改善**: リンクブロックを縦並びのレイアウト（flex-direction: column）に変更し、矢印アイコンを要素の下部に配置。
  - **一貫性の向上**: ステータスインジケーター（開催中、準備中など）も中央に配置し、視覚的な重心を整えました。
