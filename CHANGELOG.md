...
- **`main/style.css`**:
  ...
  - **解決策**: `@media (max-width: 768px)` で `html { zoom: 0.9; }` を適用。CSS `zoom` プロパティにより、`px`・`rem` 等の単位に関係なく画面全体を均一に90%スケーリング。`position: fixed` のヘッダーやボトムナビも含めて一貫して縮小される。
  - **得られた知見**: セクション単位でフォントサイズやパディングを個別調整するよりも、`zoom` で全体スケールを変更するほうが一貫性を保てる。ただし `zoom` は viewport の解釈を変えるため、メディアクエリのブレイクポイントには影響しない点に注意。
