...
## [0.5.5] SOKアトラクトモード（待機トップ画面）の実装と操作性強化 - 2026-07-21
...
- **`pos/sok.html` にアトラクトモード（待機画面）を追加**
  - 初期読み込み時（店舗側の初回起動時）は「SOKを起動する」スタートアップ画面を表示（全画面化などの初期処理に対応）。
  - 「SOKを起動する」をタップした後はアトラクトモード（待機スライドショー）へ移行し、以降の注文完了・タイムアウト時・受付再開時はすべてアトラクトモードへリセットされる完全な運用ループ仕様に調整。
  - キオスク端末としての視認性を極限まで高めるため、団体名（`storeData.name`）と企画名（`storeData.teamName`）を正確にマッピング。企画名を超巨大サイズで配置し、団体名は小さく添えるデザインに修正。これは商品選択画面のヘッダーにも適用されています。
  - 画面下部に明滅するCTA（「画面をタッチして注文」）を配置し、画面のどこをタッチしても注文メニュー (`screen-main`) に遷移。
- **カート更新時のアニメーション無効化によるちらつき防止**
  - カート（ご注文内容）に商品を追加するたびに既存の商品までアニメーション（フェードイン）してチカチカする現象を解消するため、カートアイテムのアニメーションを無効化。追加・削除が即座に反映される自然なPOSの挙動になりました。
- **無操作時の自動リセットと警告ポップアップ**
  - 商品選択画面（`screen-main`）で20秒間タッチ操作がなかった場合、自動で「ご注文を続けますか？」という警告ポップアップを表示（10秒後にアトラクト画面へリセット）。
  - QRコード表示画面のタイマーを30秒に変更（確認モーダル表示まで30秒）。
  - ※画面タッチやポップアップ内のボタン操作でタイマーは即座にリセット・継続されます。
- **受付停止（Suspended）および商品状況のリアルタイム完全同期**
  - Firestore上の `stores/{storeId}` (店舗ステータス) に加え、`items` (商品リスト・売り切れ状況) も `onSnapshot` によるリアルタイム監視へ移行。
  - 店舗側（POS/管理画面）で「売り切れ」操作や「受付停止」操作を行った瞬間、SOK画面へ即座に反映されるようになりました。
- **画像なし（`noimage.png`）表示不具合の修正**
  - 画像URLがない商品や画像の読み込み失敗時に `.loaded` クラスが付与されず、CSSの `opacity: 0` の影響で `noimage.png` が透明になってしまっていたバグを修正。表示されるようになりました。
