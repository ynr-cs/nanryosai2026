...
## [0.2.117] Mobile Scaling UI Fix & Viewport Modification - 2026-04-10
...
- **`main/style.css`**:
  - `main`要素への `zoom: 0.9` の適用を**削除**しました。
  - **原因**: `zoom`によりコンテンツコンテナ（main）の高さが縮小された結果、bodyの`padding-bottom`との計算に齟齬が生じ、一番下までスクロールしても背景が見えすぎる（ボトムナビ背後の透過背景から無地が見える、モバイルオーダー宣伝部分が途切れて下端に届かない）というレイアウト破綻が発生していました。
  - **`app-menu-overlay`** の `height: 100vh;` を `height: 100dvh; bottom: 0;` に変更し、スマホのアドレスバー等による高さの不足（一番下まで伸びない現象）を修正しました。

- **`main/app-shell.js`**:
  - `applyMobileZoom` メソッドを新設し、アプリ初期化時(`init`)に呼び出すようにしました。
  - **解決策**: スマホ（横幅 414px 未満）の場合に限り、`meta name="viewport"` の `width` を仮想的に `414` に固定するロジックを実装しました。
  - これによりブラウザネイティブのスケーリング機能が作動し、レイアウト計算を破壊することなく安全に「画面上のすべての要素を均等に縮小表示（約0.9倍）」することが可能になり、ユーザーの**「セクションごとではなく画面サイズ自体を小さくする」**要望を実現しました。
...
- **`main/style.css`**:
  ...
  - **解決策**: `@media (max-width: 768px)` で `main { zoom: 0.9; }` を適用。CSS `zoom` プロパティにより、`px`・`rem` 等の単位に関係なくコンテンツ領域を均一に90%スケーリング。`html` ではなく `main` に適用することで、`position: fixed` のヘッダー・ボトムナビ・ハンバーガーメニューへの副作用（背景の拡張、メニューが画面下まで届かない問題）を回避。
