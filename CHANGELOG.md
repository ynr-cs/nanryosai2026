...
## [0.2.146] Firebase Auth: Popup Blocker Verification Test - 2026-05-06
...
- **`pos/portal.html` & `pos/mobile-order.html`**:
  - **検証用遅延ログイン機能**: ポップアップブロックの発生条件（非同期処理によるユーザージェスチャーの喪失）を実証するため、意図的に2秒間の遅延を挟んでから `signInWithPopup` を実行するテストボタンを追加。
  - **検証ロジック**: 遅延ボタン押下時に `auth/popup-blocked` エラーが発生することを確認し、既存の「遅延なし」の実装の正当性を証明するための環境を構築。
  - **UIガイドライン**: ブロック発生時に表示される案内UIの動作テストも兼ねる。
