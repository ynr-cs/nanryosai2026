...
## [0.2.63] Hourly Growth (HoH) Display - 2026-02-19
...
- **portal.html - 売上サマリーへの前時比（HoH）表示**:
  - **前時比（%）の計算**: 直近1時間の合計（売上・注文数）を、その前の1時間の合計と比較し、増加・減少率をパーセンテージで表示するように実装。
  - **ステータスバッジ**: 増加した場合は緑の「+X% ▲」、減少した場合は赤の「-X% ▼」を表示し、一目で勢いを把握できるように変更。
  - **リアルタイム反映**: 他の統計情報と同様、注文が入るたびに自動的に再計算・更新。
