...
- **AreaTool (エリア作成ツール)**:
  - 建物とは異なる、地面に描画される「エリア」を作成・編集するツールを追加。
  - 用途: 芝生、広場、水面、駐車場など、建物のない領域を表現。
  - 機能:
    - ポリゴン描画: クリックで頂点を追加し、多角形エリアを作成。
    - 頂点編集: 既存エリアの頂点をドラッグして形状を編集。
    - 削除: エリアを個別に削除。
    - プロパティパネル: エリアの色、透明度、高さ（Y座標）を調整可能。
    - スナップ機能: 既存の建物や道路の頂点にスナップして正確な配置をサポート。
- **UI/UXの改善**:
  - ツールパネルに「エリア」ボタンを追加。
  - エリア作成中はカーソルが緑色に変化し、視覚的にモードを判別可能に。
  - エリア選択時はプロパティパネルにエリア専用の編集項目を表示。

### 変更 (Changed)

- **データ構造の拡張**:
  - `editor.js` のデータモデルに `areas` 配列を追加し、エリア情報を保存・ロードできるように変更。
  - `saveMapData` および `loadMapData` 関数を更新し、エリアデータの永続化に対応。
- **選択ロジックの統合**:
  - 建物、道路、エリアの選択状態を一元的に管理する `selectedObject` ロジックを導入。
  - 選択されたオブジェクトの種類に応じてプロパティパネルの内容を動的に切り替えるように変更。

### 修正 (Fixed)

- **背景画像キャリブレーションの永続化**:
  - 背景画像のキャリブレーション設定（幅、位置、回転、透明度）が `localStorage` に正しく保存・復元されない問題を修正。
  - `saveBackgroundSettings` および `loadBackgroundSettings` 関数を修正し、すべての設定が正しく適用されるように変更。
- **UIボタンのハイライト表示**:
  - ツールボタン（建物、道路、エリアなど）のアクティブ状態が正しくハイライトされない問題を修正。
  - `updateToolButtons` 関数を改善し、現在の選択ツールに応じてボタンのスタイルが適切に更新されるように変更。
