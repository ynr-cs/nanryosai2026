...
## 6. 設計図（`diagram.puml`）からの変更・実装保留事項

現在のシステム実装と、初期設計（`設計図/diagram.puml` 等）においては以下の仕様変更および意図的な実装保留が存在します。開発時はこの点に留意してください。（2026-02-25更新）

- **セルフオーダー (SOK) およびスマホ決済画面**:
  - `sok.html` および `pay.html` は現在 **未実装** の状態です。
- **決済システム (au PAY API)**:
  - 本格的なAPI連携は **未実装** であり、開発時はモック関数（`mockAuPayPayment`）を使用して動作テストを行っています。
- **メール送信機能 (SendGrid)**:
  - 設計に存在した SendGrid を用いたメール送信機能（受付完了メール、BAN最終通告など）は **仕様から削除** されました。
- **15分放置ペナルティの自動実行処理 (Scheduler) / BAN機能**:
