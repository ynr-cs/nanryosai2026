...
| 認証                     | Googleアカウント (`@gl.pen-kanagawa.ed.jp`) | Googleアカウント（ドメイン制限なし） | スタッフ認証のみ       |
...
```
login.html?mode=student&redirect=%2Fpos%2Fmobile-order.html
```
...
2. `users/{uid}.termsAgreedAt` に初回同意時のみタイムスタンプが記録される

初回未同意時のみ同意を取得する方針とする（第5章 5.4.2 の「未同意の場合のみ」と整合）。

#### 6.3.5 ステップ3: 店舗選択

**店舗一覧の取得**  
規約同意後、画面は店舗選択モードに切り替わる。`stores` コレクションから `isActive: true` の店舗を `getDocs` で一回取得し、画面に一覧表示する。
...
**`createOrder` の呼び出し**  
来場者が最終確認画面で「確定」を押下すると、Cloud Function `createOrder` が呼び出される。
...
**エラーハンドリング**  
以下のいずれかの状況では `createOrder` はエラーを返却する。

- 認証されていない
- BANに該当する
- 商品が販売停止になっている（`isAvailable: false`）
- 受付番号の発番に失敗した（レンジ全体の探索で空きが見つからなかった場合）
...
通知許可（FCM）は、`mobile-order.html` の規約同意画面表示前（step-notification）にて、初回または未設定時に要求される。許可が得られた場合、FCMトークンが取得され `users/{uid}` の `fcmToken` フィールドに保存される。許可が得られない場合は、`status.html` 上での目視確認のみで通知を受け取る運用となる。
...
MobileApp -> Firestore: users/{uid}.fcmToken に保存

== ステップ2: 規約同意（初回または未同意時） ==
MobileApp -> Visitor: 規約同意UI表示
Visitor -> MobileApp: チェック・「同意して進む」
MobileApp -> MobileApp: agreedTerms = true

== ステップ3: 店舗選択 ==
MobileApp -> Firestore: stores コレクション (getDocs)
...
MobileApp -> Functions: createOrder
...
SOKでのGoogle認証はドメイン制限を適用しない。`@gl.pen-kanagawa.ed.jp` 以外のGoogleアカウントでもSOK経由であれば利用可能である。
...
**段階B: 来場者のスマートフォン上での操作** 5. QRコード読み取りと `sok-to.html` のオープン 6. Google認証 7. 通知許可 8. 注文内容確認と規約同意 9. 注文確定（仮注文の本注文化）10. ステータス画面への遷移
...
3. 「SOK端末として起動」ボタンを押下
4. 自動的に新しいタブで `sok.html?storeId=店舗ID` が開かれる
5. ブラウザを全画面表示モードに切り替え

`?storeId=` パラメータにより、その端末は当日、特定の店舗のSOKとして固定される。複数台設置する場合、すべての端末で同じパラメータが使用される。

#### 6.4.4 ステップ2: メニュー閲覧と商品選択

**メニュー表示**  
`sok.html` は起動時に `?storeId=` パラメータから店舗IDを取得し、`items` コレクションを購読する。タッチ操作に最適化された大きなボタン形式でメニューを表示する。
...
**「注文確定」ボタン**  
来場者がカート画面で「注文確定」ボタンを押下すると、Cloud Function `createOrder` が呼び出される。
...
`userId` は `null` で作成される。これは本注文化（`sok-to.html` での確定）時に、来場者のGoogle認証UIDが書き込まれるまでの暫定状態である。
...
```
https://ynr-cs.github.io/nanryosai-2026/pos/sok-to.html?orderId=xxxxxxxx
```
...
#### 6.4.8 ステップ6: Google認証

**Google認証の実行**  
注文の有効性確認後、`signInWithPopup`（Googleプロバイダ）を呼び出してGoogle認証を実行する。

**UIDの紐付け**  
発行されたUIDを使い、注文ドキュメントの `userId` フィールドを更新する。この時点で、その注文の正規所有者がこのUIDのユーザーとして確定する。
...
**通知許可UI**  
Google認証成功後、Push通知の許可を要求する。

「商品の準備ができたらお知らせします。通知を許可してください」というメッセージとともに、「許可する」「あとで」のボタンを表示する。

**FCMトークンの登録**  
来場者が許可した場合、FCMトークンを取得し、`users/{uid}` ドキュメントの `fcmToken` フィールドに保存する。ユーザー用の `users` ドキュメントが存在しない場合、新規に作成する。
...
1. 認証されたユーザー（Google認証UID）を取得
2. 注文ドキュメントの `userId` が認証ユーザーのUIDと一致することを確認
3. `users/{uid}.termsAgreedAt` にタイムスタンプを記録
...
Visitor -> SOK: 「注文確定」
SOK -> Functions: createOrder (channel: "sok")
...
== ステップ6: Google認証 ==
SOKTo -> Auth: signInWithPopup (Google)
Auth --> SOKTo: UID
...
SOKTo -> FCM: getToken
FCM --> SOKTo: FCMトークン
SOKTo -> Firestore: users/{uid} 作成・トークン登録

== ステップ8-9: 規約同意・注文確定 ==
SOKTo -> Visitor: 注文内容確認・規約同意UI
Visitor -> SOKTo: チェック・「注文確定」
SOKTo -> Functions: confirmSokOrder
Functions -> Firestore: users/{uid}.termsAgreedAt 記録
Functions --> SOKTo: 完了
...
**`createOrder` の呼び出し**  
スタッフが「注文を確定する」を押下すると、Cloud Function `createOrder` (orderChannel: "pos") が呼び出される。
...
POS注文では `users` ドキュメントへの規約同意記録は行わない（第3章6節参照）。
...
== ステップ3: 注文作成 ==
POS -> Functions: createOrder (orderChannel: "pos")
Functions -> Firestore: 受付番号発番 (100-999)
Functions -> Firestore: 注文作成\n(userId: null, createdBy)
...
| --------------------- | -------------------------------- | ---------------------------------------- | ------------------------ |
| 認証方式              | Googleアカウント（学校ドメイン） | Googleアカウント（ドメイン制限なし）     | スタッフ認証のみ         |
| `userId` の値         | 学校アカウントUID                | GoogleアカウントUID                      | `null`                   |
| 規約同意              | あり（`users` に記録）           | あり（`users` に記録）                   | なし                     |
| 受付番号              | 7000番台                         | 2000番台                                 | 100〜999                 |
| 注文作成関数          | `createOrder` (channel: "mobile")| `createOrder` (channel: "sok") + `confirmSokOrder`（仮称）| `createOrder` (channel: "pos")|
